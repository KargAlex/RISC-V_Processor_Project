...
always @(posedge clk) begin
	...
	$display("Entering state.   State: %d, instr, PC: %d : %d", state, instr, PC);
		...
		MemRead <= 0;
		MemWrite <= 0;
                loadPC <= 1; // Load new PC value when transitioning to IF
		case (instr[6:0])
                    7'b0000011, 7'b0110011, 7'b0010011: begin  // LW, R-Type, I-Type
                        RegWrite <= 1;  // Write memory data(LW) or ALU result(R-type, I-type) to the register file
                    end
                endcase
            end
        endcase
